...
func (s *clientService) CreateClient(ctx context.Context, req models.CreateClientRequest) (*models.ClientResponse, error) {
	...
	if err != nil && !errors.Is(err, pgx.ErrNoRows) {
		return nil, fmt.Errorf("failed to check existing client: %w", pkg.ErrInternalServer)
	}
	
	if err == nil {
		return nil, fmt.Errorf("client with email '%s' already exists: %w", req.Email, pkg.ErrBadRequest)
	}
